      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 bg-[var(--bg-surface)] border border-[var(--border)] p-3 rounded-xl shadow-xs">
        ...
        <div className="relative flex-1">
          <input
            ...
            className="w-full bg-[var(--bg-base)] border border-[var(--border)] focus:border-[var(--violet)] rounded-lg px-3 py-2 font-mono text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none"
          />
        </div>
          ...
          {(["ALL", "PUBLISHED", "DRAFT"] as const).map((mode) => (
            <button
              ...
              className={`px-3 py-1 rounded font-mono text-xs transition-colors cursor-pointer ${
                filter === mode
                  ? "bg-[var(--bg-surface)] text-[var(--text-primary)] shadow-xs font-medium"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 max-w-xs">
                        ...
                        <div className="text-[11px] text-[var(--text-dim)] line-clamp-1 mt-0.5">
                          {post.excerpt}
                        </div>
                      </td>

                      {/* Slug */}
                      <td className="px-4 py-3.5 text-[var(--sky)] text-[11px]">
                        /{post.slug}
                      </td>
                      ...
                      <td className="px-4 py-3.5 whitespace-nowrap">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] border transition-all cursor-pointer ${
                            post.isPublished
                              ? "bg-[var(--green)]/10 border-[var(--green)]/30 text-[var(--green)] hover:border-[var(--green)]"
                              : "bg-[var(--amber)]/10 border-[var(--amber)]/30 text-[var(--amber)] hover:border-[var(--amber)]"
                          }`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${
                              post.isPublished ? "bg-[var(--green)]" : "bg-[var(--amber)]"
                            }`}
                          />
                          <span>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1 flex-wrap max-w-xs">
                          {tags.slice(0, 3).map((tag) => (
                            <span
                              key={tag}
                              className="text-[9px] px-1.5 py-0.5 rounded bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text-dim)]"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      {/* Date */}
                      <td className="px-4 py-3.5 text-[var(--text-dim)] text-[11px] whitespace-nowrap">
                      ...
                      <td className="px-4 py-3.5 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            ...
                            className="px-2.5 py-1 rounded bg-[var(--bg-elevated)] hover:bg-[var(--bg-hover)] border border-[var(--border)] text-[var(--text-primary)] text-[11px] transition-colors"
                          >
                            Edit
                          </Link>
                          <button
                            ...
                            disabled={isProcessing === post.id}
                            className="px-2.5 py-1 rounded bg-[var(--bg-elevated)] hover:bg-red-500/10 border border-[var(--border)] hover:border-red-500/40 text-red-500 text-[11px] transition-colors cursor-pointer"
                          >
                            Delete
                          </button>
                        </div>
